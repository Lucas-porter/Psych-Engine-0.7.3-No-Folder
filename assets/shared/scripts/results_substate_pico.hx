...
function resultsScreen(inst) {
	...
	switch (rank) {
		case 'PERFECT':
			bf = new FlxAnimate(275, 25);
			Paths.loadAnimateAtlas(bf, 'resultScreen/results-pico/resultsPERFECT');
			bf.anim.onComplete = () -> {
				if (bf != null) {
		...
		case 'EXCELLENT':
			bf = new FlxAnimate(375, 75);
			Paths.loadAnimateAtlas(bf, 'resultScreen/results-pico/resultsGREAT');
			bf.anim.onComplete = () -> {
				if (bf != null) {
					bf.anim.curFrame = 28;
					bf.anim.play();
		...
		case 'GREAT':
					bf = new FlxAnimate(425, 75);
			Paths.loadAnimateAtlas(bf, 'resultScreen/results-pico/resultsGOOD');
						bf.scale.set(1.1, 1.1);
			bf.anim.onComplete = () -> {
				if (bf != null) {
					bf.anim.curFrame = 40;
					bf.anim.play('start');
		...
		case 'SHIT':
			bf = new FlxAnimate(780, 420);
			Paths.loadAnimateAtlas(bf, 'resultScreen/results-pico/resultsSHIT');
			bf.anim.addBySymbol('intro', 'pico loss rank', 24, true, 0, 0);
			bf.anim.addBySymbol('loop', 'pico loss rank', 24, true, 0, 0);
			bf.anim.onComplete = () -> {
			};
		default:
			bf = new FlxAnimate(425, 75);
	...
	var songText:String = PlayState.isStoryMode ? WeekData.getCurrentWeek().storyName.toUpperCase() : (PlayState.SONG.song + artist+' Ported By UliseGas');
	...
	resultsMusic = Paths.music('results' + rank+ '-pico');
	if (resultsMusic == null) resultsMusic = Paths.music('resultsNORMAL-pico');
	var resultsIntro = Paths.music('results' + rank + '-pico-intro');
